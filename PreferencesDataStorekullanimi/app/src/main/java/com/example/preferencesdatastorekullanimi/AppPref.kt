...
class AppPref(var context:Context) {
    ...
    suspend fun okuAd() : String { //Null olabilirr hatası için String? yapabiliriz veya aşağıdaki gibi default değer atayabiliriz
    ...
    suspend fun okuYas() : Int{
        val p = context.ds.data.first()
        return p[YAS_KEY] ?: 0 //NULL ise 0 ata
    }
    ...
    suspend fun okuBoy() : Double{
        val p = context.ds.data.first()
        return p[BOY_KEY] ?: 0.0 //NULL ise 0 ata
    }
    ...
    suspend fun okuBekar() : Boolean{
        val p = context.ds.data.first()
        return p[BEKAR_KEY] ?: false //NULL ise 0 ata
    }
